...
class plotter_osc:
    ...
    def create_figure(self):
        ...
        plot_X_R = figure(plot_height=600, plot_width=600, title="X-R-Plane",
              x_range=[self.Xmin, self.Xmax], y_range=[self.Rmin, self.Rmax])
        
        plot_X_R.line('X', 'R', source=self.data_X_R_traj, line_width=3, line_alpha=0.6)
        ...
        for i, s in enumerate(self.tags_X_R_null1):
            ...
            for j, y in enumerate(s[1:]):
                if j%2 == 1:
                    plot_X_R.line(x=x, y=y, source=self.data_X_R_null1[i], line_width=3, line_alpha=0.6, line_dash='dotted', color='red')        
                else:
                    plot_X_R.line(x=x, y=y, source=self.data_X_R_null1[i], line_width=3, line_alpha=0.6, color='red')  
        
        for i, s in enumerate(self.tags_X_R_null2):
            x = s[0]
            for j, y in enumerate(s[1:]):
                if j%2 == 1:
                    plot_X_R.line(x=x, y=y, source=self.data_X_R_null2[i], line_width=3, line_alpha=0.6, line_dash='dotted', color='red')        
                else:
                    plot_X_R.line(x=x, y=y, source=self.data_X_R_null2[i], line_width=3, line_alpha=0.6, color='red')  
        ...
        plot_R_S.circle('S', 'ymin', alpha='alphapoint', source=self.data_S_R)
        plot_R_S.circle('S', 'ymax', alpha='alphapoint', source=self.data_S_R)
        plot_R_S.line('S', 'ymin', source=self.data_S_R, line_width=3, line_alpha=0.6)
        plot_R_S.line('S', 'ymax', source=self.data_S_R, line_width=3, line_alpha=0.6)
        plot_R_S.line('S', 'yss', source=self.data_S_R, line_width=3, line_alpha=0.6, line_dash='dashed')
